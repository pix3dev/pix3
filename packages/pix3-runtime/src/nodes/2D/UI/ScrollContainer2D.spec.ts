...
function createDirectChildrenScrollContainer(): {
  container: ScrollContainer2D;
  topTool: Button2D;
  bottomTool: Button2D;
  input: InputService;
...
describe('ScrollContainer2D', () => {
  ...
  it('serializes and restores scrollbar thumb and track texture refs', async () => {
    ...
    const graph = await createLoader(['res://ui/thumb.png', 'res://ui/track.png']).parseScene(
      yaml,
      { filePath: 'res://scenes/scroll.pix3scene' }
    );
