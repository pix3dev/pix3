...
0. **Input parity: mouse AND touch, always.** Every gameplay/UI interaction must
   work with both. `InputService` already unifies them via Pointer Events —
   design for taps (no hover-only affordances), use
   `scene.getPointer2DWorldPosition()` for world-space aiming/dragging, and test
   both paths before calling a feature done.
0b. **Engine-first for typical capabilities.** If a *typical game-engine
   capability* is missing (hitboxes, tweens, tilemaps, pathfinding, save
   slots…), do NOT hand-roll it inside this game — add it to the pix3
   runtime/editor as a reusable node / `core:*` component / `scene.*` service,
   model the API on the best of **Godot / Unity / Cocos Creator** (pick the
   nicest fit for pix3's node+component model; propose better if you have it),
   add a spec test, and document it in `nodes-and-systems.md` (repo `docs/` +
   the copy in `.claude/skills/pix3-game-dev/references/`). Game-specific rules
   and content stay here in `scripts/`. Example already shipped this way:
   `core:Hitbox2D` + `scene.collision2d`.
