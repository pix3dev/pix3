...
export class ScoreHud extends Script {
  ...
  static getPropertySchema(): PropertySchema {
    ...
    return {
      nodeType: 'ScoreHud',
      properties: [
        str('sourceNode', 'Source Node', 'Node that emits score-changed / lives-changed / time-changed'),
  ...
  onStart(): void {
    ...
    source.connect('score-changed', this, (...args: unknown[]) => this.setScore(Number(args[0]) || 0));
    source.connect('time-changed', this, (...args: unknown[]) => this.setTime(Number(args[0]) || 0));
  ...
  private setScore(score: number): void {
    ...
    if (label instanceof Label2D) {
      label.setText(String(this.config.scoreFormat ?? '{value}').replace('{value}', String(Math.round(score))));
    }
  }
  ...
  private setTime(seconds: number): void {
    ...
    if (label instanceof Label2D) {
      label.setText(
        String(this.config.timeFormat ?? '{value}').replace('{value}', String(Math.max(0, Math.ceil(seconds))))
      );
    }
  }
