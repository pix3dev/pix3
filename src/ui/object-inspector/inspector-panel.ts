...
export class InspectorPanel extends ComponentBase {
  ...
  private renderProperties() {
    ...
    return html`
      <div class="property-section property-section--object">
        ${this.sectionRenderers.renderInspectorSummary()} ${this.sectionRenderers.renderEditorFlagsRow()}
        ...
        ${this.sectionRenderers.renderAnimationsSection()} ${this.sectionRenderers.renderEffectsSection()}
        ${this.sectionRenderers.renderScriptsSection()}
      </div>
    `;
  }
  ...
  renderComponentPropertyInput(component: ScriptComponent, prop: PropertyDefinition) {
    ...
    if (prop.type === 'string' && prop.ui?.editor === 'audio-resource') {
      const audioPreview = this.resourcePreview.getAudioPreview(state.value);
      return html`
  ...
  renderPropertyInput(prop: PropertyDefinition) {
    ...
    if (prop.type === 'object' && prop.ui?.editor === 'texture-resource') {
      const textureValue = this.resourcePreview.toTextureResourceValue(state.value);
      const previewUrl = this.resourcePreview.getTexturePreviewUrl(textureValue.url);
      const metadata = this.resourcePreview.getTextureMetadata(textureValue.url);
