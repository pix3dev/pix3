...
- The bridge also exposes a **Codex CLI** lane at `/agents/codex/v1/*`. It uses `codex exec --json` and thread resume, advertising GPT-5.6 models through `agents[]`. Editor tools are off by default; `PIX3_CODEX_ALLOW_TOOLS=1` enables the MCP relay with Codex's full-access flag. See `tools/pix3-agent-bridge/README.md` for setup and the security implication.
