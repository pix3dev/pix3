...
- **`tools/claude-bridge/`** — standalone personal/dev utility (NOT a workspace; own `npm install` + `npm start`) exposing an Anthropic Messages-shaped endpoint on `127.0.0.1:8484`, served by Claude Agent SDK sessions (Claude Code / MAX subscription auth). Pairs with the editor's `claude-bridge` LLM provider (`src/services/llm/ClaudeBridgeLlmProvider.ts`); auth between editor and bridge is a pairing token printed by the bridge.
