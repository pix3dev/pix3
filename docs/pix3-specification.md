...
- **1.23 (2026-07-19):** Localization key extraction (6.17.3): `LocalizationExtractionService` + `ExtractLocalizationKeysCommand`/`Operation` — panel **Scan** finds unlocalized scene `label:` literals (per-item Extract → default-locale key + `labelKey` bind) and script-literal keys missing from the default table (per-item Add), then seeds missing keys into non-default locales as `""` placeholders (undoable). Runtime: empty table entries now count as untranslated and fall through the fallback chain (6.17.1).
