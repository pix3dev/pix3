...
Instance children keep their authored ids when unique, so scripts keep addressing `result-label` / `retry-button` / `cta-button` by id across the split. Overlay scenes are embedded in exports like any other asset but are excluded from the navigable scene manifest and the entry-scene picker (`ProjectBuildService.isPrefabPath`), on the same grounds as `prefabs/`: they are instantiated, not booted into. The convention is enforced by `src/templates/projects/recipes.spec.ts` (YAML: every `scenes/ui/` instance authors `visible: false`, no inline full-screen dimmer in `main.pix3scene`, no orphan overlay file) and by `ProjectTemplateScenes.spec.ts`, which loads `main.pix3scene` through the real `SceneLoader` and fails if anything sourced from `scenes/ui/` comes back visible in tree.
