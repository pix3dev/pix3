...
5. **Editor UX** — **done 2026-07-29**. `game.start-online` ("Play Online") scans the project for spawnable prefabs, installs the kind table, starts the preview relay, creates the room through pix3-cloud, joins it, and *then* enters play mode — connect-before-first-frame, so a script's `onStart` already sees `net.isOnline`. `OnlineSessionService` owns that sequence and the live session state; `pix3-online-session-card` floats it over the running game (QR, join link, roster, ping, visible entities, tick rate, Leave). Two seams changed underneath: the session now survives a scene restart and a tab⇄popout host swap (leaving is tied to play mode *ending*, not to `detachRuntime`), and `GamePlaySessionService.getNetworkService()` exposes the membership so it can exist before a runner does.
   **Minting lives in pix3-cloud** (D4): `POST /api/rooms` (creates on the fabric with the service token, mints a host token), `POST /api/rooms/:id/token` (guest-first join token — no account), `GET /api/rooms/:id`, `DELETE /api/rooms/:id` (authorised by the host token itself, so no rooms table). Configured by `ROOMS_ADMIN_URL` / `ROOMS_SERVICE_TOKEN` / `ROOMS_JWT_SECRET`; absent config answers 503 rather than half-working.
   **v0 joiner path reuses the preview relay for asset streaming**: the join link is `player.html?session=…&token=…&relay=…&room=<roomId>` and carries **no room token** — the player page mints its own guest token from the relay's origin, so a shared link cannot be replayed as somebody else's identity. The kind table travels in the relay's `session-config`, because every participant must resolve a wire `Kind` through the *same* list. Two sockets, zero new hosting — publish removes this dependency in Phase 2.
6. **Template** — **done 2026-07-29** as `samples/MultiplayerArena` (`multiplayer-arena-2d`): 8-player tag arena, no binary assets (ColorRect2D + Label2D only, so the whole project reads in a diff). Player prefab with `core:NetworkedNode` + `core:ReplicatedTransform` + `user:PlayerAvatar`; eight spawn markers seated by `clientId % markers`; the room **host** runs tag/score/round rules and broadcasts `arena:state` at 1 Hz plus one immediate extra on a real change (peer signals are quota'd at 2/s, and the sample budgets for it); quick-chat emotes instead of free text — same signal path, works on a phone with no keyboard overlay. It also runs offline as a single-player sandbox, which is what makes it usable as a test bed before a room exists.
...
**Verified 2026-07-29** against a real local `pix3-rooms` in `Auth:Mode=Jwt`, driven through the shipped mint endpoints: the minted claim set is accepted by `JwtRoomTokenValidator`, both clients get their roster, a spawn by one is visible to the other with the right owner and kind, a published move arrives dequantized to within a quantum (639.98 for 640), a peer signal is delivered once, and a despawn removes the entity. Still unverified end-to-end **in a browser**: the Play Online button, the session card and the player.html joiner — and prod needs three things before that run: the collab-server deploy, `ROOMS_*` in its environment, and the editor's origin in `Rooms__Auth__AllowedOrigins` on rooms.pix3.dev.

**Still open in Phase 1:** host migration (3c) and the signal-forwarding component (3).
