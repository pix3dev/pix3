...
Status: in progress (Phase 0 started) · Date: 2026-07-26
...
- Codegen: re-enable the MemoryPack TS generator in CI (it's the commented block at `WsServer.csproj:15-21`), publish output as **`@pix3/net-protocol`** npm package consumed by runtime and games; generate the TypeId enums too (today hand-mirrored and already drifted — `src/network/README.md` documents wrong ids). Golden-vector tests stay as the contract check.
...
**D6. Network identity: explicit netId namespace.**
Authored nodes: `netId = authored nodeId` (stable across clients via YAML; duplicate-checked at load). Runtime spawns: `netId = n:<ownerClientId>:<seq>`, threaded through the existing `instanceId` parameter of `SceneLoader.instantiatePrefab` (verified injection point, `SceneLoader.ts:418-430`). Spawned *children* are addressed as `(rootNetId, childPath)` — never global `findById`, because prefab child ids collide across spawns (fresh Map at `SceneLoader.ts:426`). The wire uses a prefab-path table with stable indices emitted into the build manifest (extend `buildSceneManifestTs`).
...
1. **`@pix3/net-protocol`**: extract `MemoryPackReader/Writer` (fix the shared-writer singleton), `Emitter`, generated messages + TypeId enums, protocol version const; CI codegen (D3).
...
3. **Components**: `core:NetworkedNode`, `core:ReplicatedTransform` (snapshot buffer + timed interpolation — replaces the demo's frame-rate-dependent lerp; payloads carry server tick numbers), `RemoteEvent` via the forwarding-component approach (no `NodeBase.emit` surgery needed) + `net.on/emit`.
...
4. **Spawn/despawn** per D6, prefab table in the build manifest; despawn → `queueFree()`.
