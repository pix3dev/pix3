...
4. **Spawn/despawn** per D6 — `netKindTable` (spawnable-prefab segment) emitted into the build manifest by `buildSceneManifestTs`; despawn → `queueFree()`. Interdependent with step 3: a replication component has nothing to bind to until spawn exists, so 3 and 4 ship together.
