...
**Статус:** **Phase A (сервер) реализована 2026-07-25** на ветке `feat/asset-store-admin` — миграция БД, `store-router.ts` (12 эндпоинтов), store-слой в `library-service.ts`, `store-validation.ts`, `library-storage.ts`; 49 спек сервера зелёные (и они наконец запускаются: `packages/pix3-collab-server/src/**/*.spec.ts` добавлены в `vitest.config.ts` — раньше серверные спеки были вне `include`). Живой смоук на настоящей sqlite+HTTP: 30/30. Phases B–E не начаты. Продолжение [asset-library.md](asset-library.md): превращаем декоративный источник «Pix3 Store» (сейчас — статический builtin-пак `public/library/`) в наполняемый сервером каталог. Писать могут только админы (`users.is_admin`), читать — все, включая незалогиненных. **Оценка:** L суммарно (фазами: сервер ~M, клиент-провайдер+админ-UI ~M, OS drag&drop ~M, lifecycle-добивка ~S–M).
...
- **`StoreUploadService.ts`** — ингест OS drag&drop и массовая заливка:
  ```ts
  export interface IngestEntry { kind: 'file' | 'directory'; name: string; file?(): Promise<File>; children?(): Promise<IngestEntry[]> }
  export interface StoreIngestPlan { bundles: StagedBundle[]; issues: string[] }  // StagedBundle = { manifest-черновик, files: Map<string, Blob>, oversize: boolean }
  captureEntries(dt: DataTransfer): IngestEntry[];   // СИНХРОННО в drop-хендлере: webkitGetAsEntry() умирает после первого await — известная ловушка
  buildPlan(entries: IngestEntry[]): Promise<StoreIngestPlan>;
  upload(plan: StoreIngestPlan, opts: { onProgress(itemId: string, done: number, total: number): void; signal: AbortSignal }): Promise<LibraryItem[]>;
  ```
  Правила группировки: верхнеуровневая **папка = один бандл** (если внутри `item.json` — уважаем его, включая `id`, что даёт идемпотентный re-upload/обновление; иначе синтез манифеста: entry — по `inferItemTypeFromPath`, name из имени папки, `status:'draft'`); **одиночный файл = один item**; **`.zip` = папка** (распаковка через `jszip` — уже в зависимостях, лениво импортируется в `PlayableHtmlBuildService.ts:163`; тем же ленивым `await import('jszip')`). Обход директорий: `FileSystemDirectoryEntry.createReader()` — `readEntries()` отдаёт ≤100 записей за вызов, читать в цикле до пустого массива. Лимиты согласованы с сервером: 100MB/200 файлов на бандл, валидируются в `buildPlan` (бандл-нарушитель помечается `oversize` и исключается из заливки с сообщением). Прогресс: `fetch` не даёт upload-progress → загрузка через `XMLHttpRequest` (`xhr.upload.onprogress`), отмена через `signal` → `xhr.abort()`; это единственное место, где ApiClient-паттерн (fetch) не подходит — честно локализуем в этом сервисе.
...
- **Phase C — OS drag&drop + массовая заливка (M):** `StoreUploadService` (entries-обход, zip, группировка, лимиты), `store-upload-dialog.ts` (мета, прогресс, отмена, file-picker fallback), интеграция в `dropAllowed`/`handleDropInto`. Бонус той же механикой: OS-drop в user-библиотеку (сейчас панель не принимает `Files` нигде).
...
- **Клиент** (vitest, happy-dom; на win32-arm64 запускать с `--pool=threads`): `StoreLibraryProvider.spec.ts` — fetch-мок: мердж сервер+fallback по id, деградация в fallback при 500/оффлайне, не-админский список; `StoreUploadService.spec.ts` — обход через интерфейс `IngestEntry` (happy-dom не имеет `webkitGetAsEntry` — тесты кормят синтетические деревья), группировка папка/файл/zip, уважение `item.json`, лимиты; `store-validation.spec.ts` — gate-матрица; `library-sources.spec` — `canEditSource`, `categoriesForSource` с declared-категориями (не сломать localStorage-путь user/team). Помнить гочу `AssetLoader.loadTexture` (сидировать textureCache, если фикстуры со сценами).
