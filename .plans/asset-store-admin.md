...
- **Phase D — добивка (S): ✅ 2026-07-26.** Сид builtin-пака в стор — админ-кнопка в тулбаре store-источника: `store-seed.ts` (чистая функция на явных deps) + обвязка в `StoreLibraryProvider.seedFromFallbackPack` (единственное место, где под рукой и пак, и сервер). Идемпотентность держится на двух свойствах, а не на флаге «уже сидили»: id пака стабильны (POST заменяет item), а курирование, которое админ успел применить (`status`/`categoryPath`), читается из **сырого** серверного индекса (`getStoreIndex({status:'all'})`, не из смерженного `list()` — там лежит сам пак) и сохраняется. Новые item'ы уходят в категорию `starter` («Starter Pack», upsert перед заливкой) и публикуются; если категорию создать не удалось или мета не проходит gate — остаются draft'ами. Лицензии пака нормализуются в SPDX (`CC0` → `CC0-1.0`, иначе gate отверг бы весь пак); сам `public/library/index.json` тоже переведён в SPDX-форму. Gallery-рендер в инспекторе уже существовал с Phase B (`renderGallery` + CSS); данные в него попадают из `item.json` бандла (`completeManifest` спредит declared-поля), UI-авторинг галереи остаётся Phase E. Audit-вьюер — **третьей вкладкой существующей веб-админки** (`packages/pix3-collab-server/src/admin/index.html`; она на **Vue 3** + CDN Tailwind, а не на Alpine, как значилось в наброске — вкладка «Журнал стора» встала в ~60 строк вместо нового Lit-компонента): страницы по 50 через `offset`, человекочитаемые метки действий с цветными чипами, `datetime('now')` нормализуется в ISO+Z перед `toLocaleString` (иначе UTC читался бы как локальное время). Заодно `listAudit` отдаёт `actorName` (LEFT JOIN `users`, чтобы запись пережила удаление аккаунта) — поле добавлено и в `ApiStoreAuditEntry`. Спека обновлена записью **1.28** в `docs/pix3-specification.md` (стор в ней не был описан вовсе — запись покрывает фазы A–D целиком); новых `.md` в `docs/` не заводим.
