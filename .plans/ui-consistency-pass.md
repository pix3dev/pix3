...
- **Ребёнок под флоу-родителем** (читаем `parent.flow.enabled`, чисто редакторно):
  вверху секции Layout — плашка `Position driven by Flow on <Parent>` с кнопкой выбора
  родителя; строки `Position X/Y` в Transform рисуются disabled с тайтлом
  «Driven by parent Flow» (идиома Unity «driven by LayoutGroup»); подблок Anchors —
  disabled с пояснением. **Перед этим проверить в `Node2D`**, действительно ли рантайм
  игнорирует якоря под флоу; если нет — это сначала решение по рантайму, а не по UI.
