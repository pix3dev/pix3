...
| **`zIndex`/`sortOrder` на Node2D** (Godot-style z-order) | 🧃🤖 | S | Сейчас порядок отрисовки 2D **только** через порядок siblings в дереве (paint order = DFS, `assign2DRenderOrder`), менять его можно лишь перестановкой узлов (agent-tool `move_node` уже это делает, undoable). Явное свойство `zIndex` в property-schema Node2D дало бы смену слоя одним `set_property` без перестановки дерева. Затрагивает `render-order-2d.ts` (учитывать zIndex поверх DFS) + editor-прокси `ViewportRenderService.assign2DVisualRenderOrder` + YAML-сериализацию. Опц. `zAsRelative` (в рамках родителя) как в Godot |
