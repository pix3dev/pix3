...
- **[localization-design.md](localization-design.md)** / **[localization-SESSION-PROMPT.md](localization-SESSION-PROMPT.md)** — i18n/l10n (Godot `tr()`-adapted). **Phase 0 + Phase 1 реализованы и верифицированы** (chrome-devtools MCP на SkyDefender, 2026-07-18): SceneRunner play-mode wiring (изолированный seed-инстанс + restore-on-stop), manifest-блок + auto-discovery `locales/*.json`, `LocalizationEditorService` (preview-инстанс, authoring API), Set­PreviewLocale/UpdateLocaleEntry Command/Operation, viewport-прокси через `getDisplayText()` + `refreshLocalizedLabels`, inspector `localization-key` виджет (статус-глиф + Extract), toolbar globe-дропдаун. **Осталось:** Phase 2 (панель локализации + локализованные спрайты `textureKey`/`stateTextureKeys` + export-эмиссия locales), Phase 3 (миграция SkyDefender + docs + yalc).
