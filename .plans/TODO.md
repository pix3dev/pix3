...
**Ревизия Flow 2026-09-24:** [prompt-to-playable-flow.md](prompt-to-playable-flow.md) §8.1 и
[flow-autopilot.md](flow-autopilot.md) §8 задают оставшийся путь. Автопилот A написан, но не
прошёл приёмку из трёх ходов; перед B добавлен A1 (телеметрия, лимиты во время хода,
ограничения записи и отмена гонок). B начата: bounded smoke запускается после каждого
автономного инкремента и перед `ready`, а воспроизводимый провал становится приоритетным `FIX:`
в `progress.md`. Терминальная проба известного рецепта (idle → финал → 120 кадров → restart)
прошла живую проверку на Tapper; визуальная пульсация, routines, полный прогон и приёмка B
остаются открытыми. В Flow уже есть
полный инспектор; фаза полировки частична.
После A1/B остаются ассеты и два рецепта, автономный вход C, стриминг, публикация и remix.

- [ ] move to constructor injection in classes
- [x] **закалка агентского пайплайна по итогам Carrom** (Сложность: M) — _[agent-pipeline-hardening.md](done/agent-pipeline-hardening.md), по разбору [carrom-agent-pipeline-review.md](done/carrom-agent-pipeline-review.md). Оба пайплайна: внешний агент (`src/templates/agent/` + `docs/*.md`, которые копируются в каждый новый проект) и встроенный (`agent-skills/`, тулы, off-package-заметки). **Главная мысль §2 разбора:** всякая фраза агентской прозы, утверждающая что-то про форму дерева, — это кэш, который никто не инвалидирует; поэтому каждая правка прозы идёт в паре со спекой, выводящей утверждение из исходников заново. Сделано: `OffPackageTopic.claims` + страж в `engine-source.spec.ts` (проверено — падает); новый `agent-reference-docs.spec.ts` (нашёл **12 из 35** типов нод без секции в `node-types-reference.md` — включая `ColorRect2D`, единственный сплошной 2D-примитив, на который обе скилы сами же и посылают) — все 12 написаны по живым схемам; **безбраузерный прогон сцены** `@pix3/runtime/testing` (`createHeadlessGame`) — Carrom целиком за ~3 с, и тут же выяснилось, что очевидное утверждение «выстрел во что-то попал» проходит и на полностью неиграбельной сборке, различает только направление; `project.recents()`/`project.open()` в дев-мосте (открытие проекта больше не требует клика человека); `read_errors` очищен от 13 благих ошибок на открытие проекта (**проверено живьём: 13 → 0**); `fs_write` с `encoding:'base64'`; `juice.shake` больше не молчит вхолостую. **Осталось:** страж на соответствие таблиц свойств схемам, сбор 14 расхождений API из §5 разбора, тулчейн в шаблоне проекта_
...
- [ ] **автопилот Flow — A1 (укрепление) → B (плейтестер) → C (автономный вход)** (Сложность: M) — _Фазы A, A1 и B полностью закрыты ([flow-autopilot.md](flow-autopilot.md) §8–9). Приёмка фазы A пройдена живьём: 3 последовательных автономных инкремента без вмешательства человека на реальном LLM-провайдере (agy через мост), автоматическая остановка по бюджету, smoke-проверки и отчёт `design/autopilot-<runId>.json`. В A1 закрыты каноника путей, гарды перезаписи, отмена старого хода при смене проекта, блокировка платных тулов и остановка batch на ошибках. В B поддержаны и проверены скан контролов и routines на всех рецептах (`recipe-tapper-2d`, `recipe-bouncer-2d`, `recipe-arena-2d`, `recipe-blank-2d`). Осталась фаза C (автономный вход: vision, советник, ?$-бюджет)_
...
- [ ] **планнер не попадает в рецепт, который уже есть** (Сложность: M) — _P1: промпт «тапалка
  ...
  `FALLBACK_RECIPE_ID`, тогда ещё `recipe-arena-2d`, вместо более близкого `recipe-bouncer-2d`,
  и это стоило брифа в 1679 строк, 124K входных токенов на первом вызове и единственного прогона,
  который так и не сошёлся. **Второй симптом смягчён**: по жалобе «вайб почти всегда падающие
  кружки и звёзды» (2026-09-20) `FALLBACK_RECIPE_ID` теперь = `BLANK_RECIPE_ID` (`recipe-blank-2d`)
  — молчание планировщика больше не утверждает жанр, которого идея не называла, а сразу отдаёт
  болванку со счётом/жизнями/HUD. Первый симптом (готовый рецепт есть, но планировщик выбирает
  blank/не тот) остаётся открытым_
