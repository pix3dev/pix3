...
- [ ] **автопилот Flow — A1 (укрепление) → B (плейтестер) → C (автономный вход)** (Сложность: M) — _основа фазы A в `main`, приёмка трёх автономных инкрементов открыта ([flow-autopilot.md](flow-autopilot.md) §8–9). Живой прогон показал два стоп-критерия, без которых автономия сжигает квоту впустую: «сцена не стартует» (25× `play_start → ok:false` при `project.status !== 'ready'`) и **кап хопов на один ход** (52+ хопа на доказательство очистки линий). Попутно починен залипавший тайп-чек проекта (`ProjectDiagnosticsService` держит модели между прогонами)_
