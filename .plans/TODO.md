...
- [ ] **режим Flow — «промпт → играбельный прототип → ссылка»** (Сложность: L) — _дизайн готов, реализация не начата: [prompt-to-playable-flow.md](prompt-to-playable-flow.md). Фаза 0 (закалка харнесса: таймаут LLM-запроса, advisor-триггер от loop-breaker'а, контекст-менеджмент со сжатием в `design/*.md`, тул `ask_user`, гард на полную перезапись файлов, стабильный кэш-префикс) — первая, делается независимо и улучшает Agent-таб в Studio сама по себе; затем срез «промпт → диалог → играется», публикация, ассет-конвейер_
