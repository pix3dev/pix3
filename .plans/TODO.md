...
- [ ] **тайловые уровни: `TileMap2D` + `.pix3tileset` + кисть** (Сложность: L; играбельный срез — M) — _спека [tilemap-2d.md](tilemap-2d.md), P2 (🤖). Сегодня тайлов нет вообще, и это не закрытая развилка, а слепое пятно: в аудите [cross-engine-ideas.md](cross-engine-ideas.md) TileMap не упомянут ни гапом, ни skip'ом (`TiledSprite2D` — nine-slice внутри одного спрайта, к карте отношения не имеет). Обоснование приоритета — не паритет с Godot, а agent pipeline: уровень как «данные на нодах» стоит агенту ~12 строк YAML и двух вызовов тулов НА ОБЪЕКТ, тот же уровень в ASCII — 20 строк, правится одним `str_replace` и диффится в git построчно; попутно разблокирует рецепт `platformer-2d`, обещанный фазой 3 Flow и не существующий. **Фаза 1 (рантайм и формат, M):** `TileSetResource`/`loadTileSet` по образцу `.pix3anim`, чанковый мешер `TileMapView` (общий для ноды и редакторского прокси, как `SpineSkeletonView`), слияние solid-клеток в прямоугольники как N `Collider2DSource` на ОДНОМ статическом теле, проводка `SceneLoader`/`SceneSaver`/экспортных таблиц. **Фаза 2 (присутствие в редакторе и агентский путь, M−):** Create-команда, прокси, пикинг, редактор рядов в инспекторе, сводка карты в `scene_tree`, eval-сценарий. **Фаза 3 (кисть, M) — за воротами:** нужен хотя бы один живой прогон, где человеку правка `rows` текстом оказалась ощутимо больнее перетаскивания `ColorRect2D`; без этого фаза уезжает во `frozen/`, формат к этому готов. Тула рисования для агента в v1 нет — формат спроектирован под `set_property rows`/`str_replace`. Найденная по дороге ловушка вне плана: `SceneSaver` зовёт `stringify(document, { indent: 2 })`, а у `yaml@2` дефолтный `lineWidth: 80` — любой ряд шире 80 клеток сложится переносом; нужен `lineWidth: 0`_
...
- [x] **встроенная 2D-физика: `core:PhysicsBody2D` + `core:Collider2D` + `scene.physics2d`** (Сложность: L) — _сделано на `feat/collision-polygons-2d-physics`, спека [physics-engine.md](physics-engine.md). Три фазы: **0** — полигоны коллизий (`core/collision-shapes-2d.ts` с SAT и выпуклой декомпозицией, шейп `polygon` у `Collision2DService`/`core:Hitbox2D`, `polygonSource: 'frame'` наконец-то читает обводку, которую Sprite Editor рисовал с §9.12.2 и **никто не потреблял**, плюс редактирование полигона прямо во вьюпорте одной записью в undo); **1** — солвер (`Physics2DService`, `core:PhysicsBody2D`/`core:Collider2D`/`core:PhysicsWorld2D`, сенсоры, swept-CCD, сон по островам контактов, блочный солвер двухточечных манифолдов, спекулятивные контакты, `raycast`/`overlapCircle`/`overlapRect`, сигналы, стриппинг из экспортов без физики); **2** — `moveAndCollide`/`moveAndSlide` (роль CharacterBody2D), revolute joint с лимитами и мотором, интерполяция поз между фиксированными шагами, капсула, тоггл «Show collision shapes». Солвер рукописный импульсный, Rapier в 2D-путь так и не зашёл; `core:Hitbox2D` остался query-only слоем. Приёмка держится: 20-боксовый стек стоит без бокового дрейфа 30 с при 1/60. **Осознанно не сделано:** `recipe-bouncer-2d` остаётся на своём солвере — его бамперы с `restitution` 1.05/1.3 _добавляют_ энергию, а движок клампит в [0, 1] как Box2D и Godot, так что порт судится только игрой, а не тестом; broadphase намеренно раздельный (два слоя индексируют разное, замеров проблемы нет); фаза 3 (3D-обёртки над Rapier) demand-gated, пока DeepCore единственный потребитель. **Известные дыры** (§9 спеки): швы вогнутых коллайдеров без ghost vertices, CCD только circle-vs-static (быстрый _полигональный_ body туннелирует), группы фильтруют запросы, но не гейтят коллизии — layer/mask матрицы в v1 нет_
...
- [x] **Peek: скрыть/показать группу объектов без дерева** (Сложность: M) — **сделано 2026-09-10** — _план [done/editor-peek-visibility.md](done/editor-peek-visibility.md). Фаза 0: пер-юзерская несериализуемая маска видимости вне undo (персист в localStorage по пути сцены, никогда в `.pix3scene`), чипы автовыводятся из верхнеуровневых ветвей сцены, клик = toggle, ⌥-клик = solo **гашением** до 25% альфы, несбрасываемая пилюля «N hidden · Show all», инструмент агента `peek` + поле `hiddenByEditor` во всех выводах агента, предупреждение на Export/Download HTML. Механизм — аксессор `visible` на `NodeBase` (`selfVisible && !hiddenByEditor`); две ловушки: зеркалирование `properties.visible` в сеттере (иначе ломается реактивный слой схемы, где `visible` сейчас per-instance аксессор) и `Object3D.clone()`. Заодно переименовать заголовки `ToggleLayer2D/3DCommand` в «Show 2D»/«Show 3D» — это фильтры размерности, а не слои. Фаза 1 (теги как таксономия) заморожена: [frozen/scene-tags-visibility.md](frozen/scene-tags-visibility.md). **Правка по факту:** «две ловушки» выше — только одна; реактивный слой схемы `visible` никогда и не оборачивал. Итоговые расхождения — в разделе «Что разошлось с планом при реализации» самого плана_
