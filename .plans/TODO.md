...
- [ ] **Peek: скрыть/показать группу объектов без дерева** (Сложность: M) — _план [editor-peek-visibility.md](editor-peek-visibility.md). Фаза 0: пер-юзерская несериализуемая маска видимости вне undo (персист в localStorage по пути сцены, никогда в `.pix3scene`), чипы автовыводятся из верхнеуровневых ветвей сцены, клик = toggle, ⌥-клик = solo **гашением** до 25% альфы, несбрасываемая пилюля «N hidden · Show all», инструмент агента `peek` + поле `hiddenByEditor` во всех выводах агента, предупреждение на Export/Download HTML. Механизм — аксессор `visible` на `NodeBase` (`selfVisible && !hiddenByEditor`); две ловушки: зеркалирование `properties.visible` в сеттере (иначе ломается реактивный слой схемы, где `visible` сейчас per-instance аксессор) и `Object3D.clone()`. Заодно переименовать заголовки `ToggleLayer2D/3DCommand` в «Show 2D»/«Show 3D» — это фильтры размерности, а не слои. Фаза 1 (теги как таксономия) заморожена: [frozen/scene-tags-visibility.md](frozen/scene-tags-visibility.md)_
