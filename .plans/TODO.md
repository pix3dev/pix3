...
- [ ] drag-to-scrub в инспекторе анимационного ассета (Сложность: S) — _отдельная поверхность `inspector-section-renderers.ts` (FPS, duration multiplier, anchor X/Y, bounding box) — 8 сырых number-инпутов с `@change`; их правки не проходят через preview/commit-путь property-схемы_
