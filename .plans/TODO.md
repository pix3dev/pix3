...
Всё ниже найдено во время [vibe-vs-chat-gap.md](vibe-vs-chat-gap.md) и намеренно **не** чинилось
по ходу — правка харнесса обнулила бы сравнимость прогонов. Порядок — по измеренной цене.

- [ ] **`create_node` молча срывает уже привязанные компоненты со сцены** (Сложность: M) — _самый
  дорогой из найденных: `GameRules`/`ScoreHud` пропадали с `game-root`/`hud` после добавления
  ноды в **3 прогонах из 4** (P1, P2, P4), и каждый раз агент тратил шаги на то, чтобы заметить
  и вернуть их через `add_component`. Похоже на перезапись узла сцены целиком вместо точечной
  вставки. Чинить первым: налог платится в каждом прогоне_
- [ ] **подача первого кадра: прототип выглядит как цветные прямоугольники** (Сложность: L) —
  ...
  дешевле, чем кажется. Обоснование и цифры — §5 замера_
...
- [ ] **верификация состоянием слепа к ориентации и вообще к «похоже ли это на игру»**
  ...
  человека_
- [ ] **кап тул-итераций рвёт ход посередине задачи** (Сложность: S) — _2 инкрементных хода из 6
...
- [ ] **`t_idea` 10–16 с без единого обращения к модели** (Сложность: S) — _создание проекта и
  разворот шаблона идеи съедают четверть-половину всего 36-секундного бюджета, за который эталон
  отдаёт готовую игру. Единственный отрезок разрыва, который сокращается без модели, рецептов и
  арта_
