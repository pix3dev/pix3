...
- [ ] **`t_idea` 10–16 с без единого обращения к модели** (Сложность: S) — _**замерено
  2026-09-01, и замер снимает гипотезу**: путь «клик Make → дизайн-док на экране» стоит
  **94 / 77 / 245 мс** (три живых прогона через prompt-hero, dev-сборка, OPFS). Разбивка при
  94 мс: OPFS-каталог проекта 8, каталоги шаблона 3, манифест 10, чтение+запись текста шаблона 9,
  агент-оверлей (~110 КБ, 8 файлов) 11+26, док идеи 10, рендер `pix3-idea-doc` 8. То есть
  создание проекта и разворот шаблона — **не** те 10–16 с. Инструментирование живёт в
  `src/services/flow/idea-timeline.ts`: `console.info` с разбивкой, `performance.measure` для
  трейса и `window.__pix3IdeaTimeline()` для автоматики. **Попутно воспроизведена ровно та
  ловушка, о которой писал сам замер** («раннер ловил кнопку прошлого проекта»): `pix3-idea-doc`
  прошлого проекта перерисовывается в первых кадрах и закрывает секундомер досрочно — поэтому
  финиш арминтся только после открытия нового проекта (`armCompletion`). **Осталось**: снять
  разбивку в тех же условиях, что и ручной замер (холодный профиль/холодная загрузка страницы) —
  разница между `totalMs` и суммой фаз и будет искомым отрезком_
