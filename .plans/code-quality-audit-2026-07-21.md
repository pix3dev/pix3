...
1. **Ручной смоук-тест `ViewportRenderService`** (§12) — 2D paint-order/mipmaps в живом редакторе после decomposition'а. Не покрыто автотестами, не подтверждено как сделанное перед мерджем PR #21.
2. **§2.2 — дубликаты editor-proxy vs runtime-node в `ViewportRenderService`** (установка материалов/renderOrder) — отмечено как «проверить точечно», метрикой не подтверждено, никогда не разбиралось отдельно. Актуальность может измениться после decomposition'а (§12) — коллаборатор `Viewport2DProxyRegistry` теперь изолирует этот код, что может облегчить проверку.
