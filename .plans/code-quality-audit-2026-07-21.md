...
| P3 | Декомпозиция `inspector-panel` (§3 №2) | сред | M | Opus | ✅ (ветка `refactor/inspector-panel-decomposition`, §11) |
| P4 | Декомпозиция `ViewportRenderService` (§3 №1) | **выс** | L | Fable стратегия → поэтапно | ✅ (ветка `refactor/viewport-render-service-decomposition`, §12) |
| P4 | Bundle-анализ + вывод по §5 | низ | S | я | ✅ (ветка `perf/bundle-runtime`, §9, смержена) |

**Следующий шаг:** обе декомпозиции (§11, §12) готовы и ждут ревью/мерджа в `main`; в аудите не осталось незапланированных пунктов.
...
**Не сделано:** мерж в `main` — ветка оставлена локально на ревью.
...
**Не сделано:** мерж в `main` — ждёт решения пользователя. Рекомендация: перед мерджем — ручной смоук-тест 2D paint-order/mipmaps в живом редакторе (единственное, что не покрывает автотест-сьют), см. §8 Item 1 про "ручная проверка".
