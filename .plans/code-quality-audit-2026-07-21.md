...
| P4 | Декомпозиция `ViewportRenderService` (§3 №1) | **выс** | L | Fable стратегия → поэтапно | ✅ (ветка `refactor/viewport-render-service-decomposition`, §11) |
| P4 | Bundle-анализ + вывод по §5 | низ | S | я | ✅ (ветка `perf/bundle-runtime`, §9, смержена) |

**Следующий шаг:** P3 осталось — декомпозиция `inspector-panel` (не требует дизайна от Fable, план §3 №2 достаточен для старта).
...
## 11. P4 — декомпозиция `ViewportRenderService` (2026-07-22) — ветка `refactor/viewport-render-service-decomposition`
