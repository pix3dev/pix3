...
Весь приоритизированный план (§6, P0–P4) смержен в `main`: PR #18 (red-build fix + knip + барель + микро-dead-exports), #19 (bundle perf), #20 (`FrameSequencePlayer`), #21 (`ViewportRenderService` decomposition), #22 (`inspector-panel` decomposition), #23 (dead-export cleanup follow-up). Все шесть — с зелёным gate (`tsc`, `vitest`, `eslint`) на момент мерджа.
...
3. ~~**§4.3 / §8 хвост — 94 плоских сервиса в `src/services/`**~~ ✅ Сделано (PR #24, 2026-07-22) — см. §14.
4. ~~**§9 — lazy-resolve DI паттерн** (`@injectLazy`)~~ ✅ Сделано (PR #25, 2026-07-22) — см. §14.
5. ~~**§9 — collab-стек (yjs/hocuspocus, 414KB) в `ProjectStorageService`**~~ ✅ Сделано (PR #26/#27, 2026-07-22) — см. §14.
6. ~~**§1.3 — unused exports/types (knip)**~~ ✅ Пройдено 2026-07-22, ветка `chore/dead-export-cleanup-followup` — см. ниже.

Из пунктов 3–5 (архитектурные, для Fable) ничего не запланировано как обязательное.
...
### §1.3/§1.2 повторный проход — ветка `chore/dead-export-cleanup-followup` (закрыто)

knip до: `Unused exports (65)`, `Unused exported types (43)`. После триажа (Opus-агент, каждый кандидат проверен grep'ом на реальное отсутствие ссылок вне известных ложных категорий — Lit `@customElement`-классы, Command-классы, структурно используемые типы/бареля) — знип после: **29 / 39**. Удалённое подтверждено дважды: агентом (gate после каждого батча) и сессионной моделью независимо (`tsc --noEmit` 0, `vitest run` 1302/1305 — те же 3 pre-existing fail, `eslint` на затронутых файлах чист после точечной правки 3 prettier-нитов, оставленных агентом).

**Удалено целиком** (мёртвые функции/значения без ссылок): фабрики-обёртки в `DeleteObjectCommand`/`DuplicateNodesCommand`/`GroupSelectedNodesCommand`/`node-placement.ts`/`SelectObjectCommand.ts`, `ApiClient.getManifest/getBaseUrl`, `resolveCloudProjectCacheService`, `resolveDialogService`, `resolveAtlasCacheStore`, `stripImageExtension`, `library-sources.getLibrarySource`, `library-types.categoryForItemType` (+ приватная map) / `LIBRARY_SCOPES` / `LIBRARY_SCOPE_LABELS`, `hierarchy-validation.is3DNode`; типы `TargetTransformState` (заменён на `TargetTransformParams`), `TemplateScheme`/`TemplateLookupError`, `ImageTemplateId`, `ScriptCreationOptions`, `property-editors.SizeValue`, `ResourceManager.ReadResourceOptions`.

**`export` → module-private** (использовались только внутри своего файла, включая наш `getScrubSensitivity` из §11): ~20 констант/функций в `ProjectManifest.ts`, `clip-edit-utils.ts`, `UpdateEditorSettingsOperation.ts`, `NudgeNodesCommand.ts`, `prefab-utils.ts`, `hierarchy-validation.ts`, `AgentSettingsService.ts`, `GltfBlobLoader.ts`, `ProjectTemplateService.ts`, `NodeWatchRecorder.ts`, `image-ops.ts`, `library-path-remap.ts`, `library-search.ts`, `library-sources.ts`, `script-diagnostics-format.ts`, `timeline-geometry.ts`, `grouped-asset-tree.ts`, `monaco-runtime-libs.ts`, `inspector-property-renderers.ts`, `asset-drag-drop.ts`. Command-классы `SetEditorCameraProjectionCommand`/`SetPreviewCameraCommand` тоже демоутнуты (вызываются только через свои фабрики-функции, не по классу — проверено grep'ом по всему `src/`, включая спеки).

**Оставлено намеренно** (проверено, не мёртвый код): все Lit `@customElement`-классы (29 из оставшихся unused-exports — тег-регистрация, знип не видит); WebSocket-протокол `remote-preview/protocol.ts` и discriminated-union члены `agent-eval.ts` (внутреннее использование как параметр-типы); барели `state/AppState.ts`/`state/index.ts` (структурный type-only реэкспорт, как и предполагал §1.3); `IconService.IconSizeValue`/`FocusRingService.FocusRingServiceOptions` (публичные API-алиасы, 0 ссылок, но низкая ценность удаления против риска — оставлены с пометкой).

**Смержено:** PR #23 → `main` (2026-07-22), Copilot-ревью без замечаний (42/42 файлов чисто).

---

## 14. Закрытие архитектурных хвостов §13 п.3–5 (2026-07-22) — PR #24, #25, #26/#27

Процесс: дизайн/аудит — сессионная модель (Fable), механика — Opus-агенты, каждый PR — Copilot-ревью → правки → мердж. Все гейты (`tsc` 0, `vitest` 1302/1305 с теми же 3 pre-existing fail, `build`, `knip` без регрессий) держались на каждом шаге и перепроверялись сессионной моделью независимо от отчётов агентов.

### §4.3 — доменная группировка сервисов + аудит mutation gateway (PR #24)
- **Группировка:** ~100 loose-файлов `src/services/*.ts` разложены по 17 доменным папкам (`core`, `scene`, `project`, `cloud`, `collab`, `assets`, `scripting`, `play`, `export`, `editor`, `animation`, `localization` — новые; `agent`/`image-gen`/`bg-removal`/`library`/`viewport` — доукомплектованы). В корне `src/services/` файлов не осталось (политика в AGENTS.md). Два runtime-спека (`InputService.runtime.spec.ts`, `Joystick2D.runtime.spec.ts`) переехали в `packages/pix3-runtime/src/core/`. ~560 alias-импортов + ~290 относительных + 12 `vi.mock` переписаны кодмодом; относительные `import.meta.glob`/`?raw`/`?url` в `PlayableHtmlBuildService`/`ProjectBuildService`/`ProjectTemplateService`/`template-data.ts` получили +1 уровень глубины **включая bracket-lookups по ключам глобов** (кодмод их не видит — правились руками, главный риск-спот).
- **Аудит «в обход Command→Operation»:** реальных обходов НЕТ. `HistoryManager` достижим только через `OperationService`; блуждающих `.perform()` нет; все 15 сервисов, пишущих `appState` напрямую (Auth/Collaboration/Router/ProjectLifecycle/EditorTab/…), пишут **session/UI/infra-состояние**, не undoable document-состояние. Правило формализовано в AGENTS.md («gateway = document state; session state пишет владеющий сервис»).
- **Ревью:** Copilot отказался (343 файла > лимит 300) → заменён независимым adversarial-ревью Opus-агентом по 6 поверхностям риска (не-путевые изменения, глубины глобов, vi.mock-пары, stale-строки, fixture-перезаписи, docs): approve, 0 находок.

### §9 lazy-DI — `@injectLazy` (PR #25): eager main −280KB
- `src/fw/di.ts`: `@injectLazy(() => import('…').then(m => m.ServiceClass))` + тип `LazyService<T>` — свойство становится async-аксессором; **кэшируется только module import, резолв через контейнер на каждый вызов** (re-registration наблюдаем, transient не ломается — это исправление по ревью Copilot, первая версия кэшировала инстанс). Ошибки оборачиваются host/property-контекстом как в `@inject`; guard на «loader вернул не конструктор» (второе замечание Copilot).
- Применено к 3 eager-рёбрам: `ProjectDiagnosticsService`→`MonacoIntelliSenseService` (тянул monaco-runtime-libs) и оба `ExportPlayable*Command`→`PlayableHtmlBuildService` (остальные импорты оттуда — type-only). `code-tab.ts` не трогали (уже lazy-чанк).
- **Замер:** eager `main-*.js` 2 407 615 → 2 127 217 B; новые lazy-чанки Monaco (170 879 B) + Playable (111 121 B) сходятся с падением 1:1; маркер-строки обоих модулей в eager-чанках отсутствуют.

### §9 collab-стек (PR #26 + reland #27): eager main ещё −125KB
- **Ключевой факт, снявший «архитектурный» статус:** `Y.Doc` создаётся ТОЛЬКО в `CollaborationService.connect()` — соло/локальные сессии CRDT не трогают вовсе; а `ProjectStorageService` использует `yjs` только в type-позициях (утверждение аудита «Y.Doc используется даже вне cloud-бэкенда» не подтвердилось → `import type`).
- `connect()` стал `async` и грузит yjs+hocuspocus через `import()`; epoch-guard отменяет устаревший in-flight connect (включая `disconnect()` во время загрузки). Sync-API консюмеров не изменился (`getYDoc()` и так возвращал `null` вне сессии; undo/redo-фоллбек `OperationService` не тронут). `SceneCRDTBinding` резолвится через динамический `import()` в join/session-флоу; его eager-регистрация в `register-runtime-services` удалена (`@injectable()` самодостаточен). Правило «никаких eager top-level yjs value-импортов вне SceneCRDTBinding.ts» — в AGENTS.md.
- **Замер:** eager `main-*.js` 2 127 217 → 2 002 320 B; lazy-чанки yjs (93 763) + hocuspocus (42 517) + SceneCRDTBinding (4 621).
- **Процессная ловушка (#27):** stacked-PR #26 (база `perf/lazy-di`) был смержен В СВОЮ БАЗУ уже после того, как #25 ушёл в main → контент не попал в main, повис на ветке. Reland-PR #27 (тот же дифф) закрыл дыру. Урок: **стек мержить строго снизу вверх** (#24→#25→#26), либо перед мерджем верхнего PR убедиться, что база ещё не смержена.

**Суммарно:** eager JS −405KB (2 407 615 → 2 002 320 для большого main-чанка). Аудит 2026-07-21 полностью закрыт: все пункты §6 и все хвосты §13.
