...
| `recipe-arena-2d` | аватар в ограниченном поле, спавнер сыпет пикапы/опасности, касание = очки/урон | доджеры, коллекторы, top-down выживание, **змейка**, (свопом движения) раннеры |
...
`recipe-arena-2d` — **fallback**: неизвестный `recipeId` от planner'а разворачивается в него,
и в `design/brief.md` пишется строка «planner asked for `<id>`; started from arena-2d».
