...
**Статус:** реализовано (MVP) на 2026-07-12; ручная проверка в живом редакторе закрыта 2026-08-11 —
5 из 6 пунктов §5 подтверждены данными, шестой (`Move to Folder`) упирается в нативный диалог и
остаётся за человеком. По ходу найден один дефект активации при создании через промпт «Unsaved
Changes» — см. §5.1. **Оценка:** M.
...
### 5.1 Журнал проверки — 2026-08-11, живой редактор (chrome-devtools MCP, dev-сервер :8123)

Пять пунктов из шести закрыты; шестой упирается в нативный диалог, который MCP не водит.
Всё судилось по состоянию и байтам в OPFS, не по скриншотам.

**1 — создание без пикера: ✅.** Диалог на шаге «Storage» открывается с активным `In Browser`
(`backend-option--active`) и копирайтом «Instant start — the project is stored inside this browser»
(§7 требовал именно формулировку «stored inside this browser»). `window.showDirectoryPicker`
подменён перед нажатием Create на счётчик вызовов — после успешного создания счётчик **0**, пикер
не открывался ни разу. В OPFS легло полное дерево шаблона `empty-3d`: `scenes/main.pix3scene`
(2879 б), `sprites/pix3-logo.png` (609 824 б), `pix3project.yaml`, `.pix3/template.json`,
`.claude/skills/…`. Редактор открылся на `scenes/main.pix3scene` — все 9 нод шаблона в дереве,
ошибок нет.

**2 — переживает reload: ✅.** Нода переименована через мутационный шлюз, `Ctrl+S` — правка
оказалась **в файле внутри OPFS** (читал `main.pix3scene` напрямую из handle: `name: OPFS Survivor
Box`). После полного reload страницы проект поднялся сам, без единого промпта, имя на месте,
`errors()` пуст.

**3 — play mode и ассеты: ✅.** `play.start()` → рантайм-дерево построено, у `Logo Sprite`
`material.map.image.width = 1024` — то есть PNG **из OPFS** реально прочитан, декодирован и залит в
GPU, а не просто «нода создалась». Ноль ошибок за прогон.

**4 — recents: ✅.** На welcome карточка `OPFS Verify B` с бейджем **Browser** рядом с `Local`-ными,
открытие по клику работает (проект активировался, ошибок нет).

**5 — Move to Folder: частично (гейтинг ✅, сам перенос не проверен).** Пункт меню
`File → Move Project to Folder…` **disabled** на local-проекте (SkyDefender) и **enabled** на
browser-проекте — гейтинг ровно тот, что задуман. Дальше нужен `showDirectoryPicker`, а это нативный
диалог: CDP его не закрывает, поэтому «файлы на диске / backend стал local / OPFS-копия удалена»
остаётся на ручную проверку человеком. Это единственный незакрытый пункт.

**6 — deep-link: ✅.** `#editor?local=<id>` browser-проекта поднимает его после reload; проверено
дважды подряд.

**Найденный дефект (наблюдался один раз, воспроизвести не удалось).** Первая попытка Create прошла
через диалог «Unsaved Changes» (в редакторе висела грязная вкладка Sprite Editor). После «Don't
Save» проект в OPFS **создался целиком и валидным**, но активация не состоялась: в recents записи
не появилось, диалог вернулся в состояние «Create», а директория осталась в OPFS сиротой. Трейл:
сначала `[AssetLoader] Failed to read blob for res://sprites/pix3-logo.png: Resource not found`
(файл при этом лежит в OPFS нового проекта), сразу за ним
`[LoadSceneCommand] Scene load failed: res://src/assets/scenes/main.pix3scene` — это путь
**предыдущего** проекта. Обе ошибки складываются в гонку порядка при переключении проекта: ассеты
новой сцены запрашиваются раньше, чем переключился корень (или сессия вкладок старого проекта
восстанавливается уже против нового корня), а `await` на подтверждении расширяет окно. Вторая
попытка, без грязных вкладок, прошла безупречно. Корень не доказан — нужен отдельный заход с
повторением на грязной вкладке.

**Не дефект OPFS:** открытие проекта с welcome не восстанавливает вкладку сцены (дерево пустое,
хотя сохранённая сессия в `pix3.projectTabs:<id>` содержит `res://scenes/main.pix3scene`).
Проверено на local-проекте — ведёт себя ровно так же, значит поведение общее для обоих бэкендов и к
browser-storage отношения не имеет.

**Уборка:** оба тестовых проекта удалены из OPFS вместе с записями в recents и сохранёнными
сессиями вкладок; OPFS вернулся к исходным пяти проектам, редактор — к SkyDefender.
