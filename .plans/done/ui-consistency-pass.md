...
Статус: **реализовано целиком** (шаги G1–G12), ветка `feat/ui-consistency-pass`, 2026-09-08.
Повод: редакторы открывались то из `View`, то из `Tools`; Align включался кнопкой, Flow —
чекбоксом; `Disable`/`Remove` в компонентах и эффектах были текстовыми ссылками.

Хвост: `cd packages/pix3-runtime && npm run yalc:publish` + `yalc update` у консьюмеров —
G11 переименовал в рантайме лейблы (`Anchor`→`Anchors`, `Cross Align`→`Cross Axis`), и до
публикации консьюмеры (DeepCore) видят прежние подписи. Ничего не сериализуется, миграция
сцен не нужна.

Что нашлось и починилось по дороге, помимо задуманного:

- Два фантомных top-level меню «Create/2d» и «Create/3d» — пять команд висели в незнакомых
  `menuPath`, а `Create` при этом синтезировался из реестра нод (G1).
- Меню строило секции один раз в `connectedCallback`, до регистрации команд шелла, и
  навсегда отдавало устаревший снапшот — секция, зарегистрированная позже, не получала
  кнопки вообще (G7).
- Q/W/E/R с клавиатуры не подсвечивали кнопку в тулбаре: у меню и тулбара были разные
  источники правды (G4).
- `pix3-panel { height: 100% }` — незаскоупленное правило из стилей инспектора било по
  **каждой** панели редактора; оказалось объявлено дважды, вторая копия держала три панели
  без своих правил (G8, G9).
- `PropertySchema.groups[].expanded` было объявлено и не читалось: 12 файлов рантайма уже
  проставляли его вслепую (G10).
- `CreatePrefabInstanceCommand` никогда не регистрировался, поэтому его меню `insert` не
  существовало; и запускаться без `prefabPath` он не может (G1).
...
4. Подменю — сегменты пути: `menuPath: 'node/align'`. Фаза 1 рендерит подменю как
   ...
   Уточнено при реализации G7 (фаза 2 сделана, озаглавленные группы удалены):
   - Панели — **соседи в портале**, а не вложенные элементы: у дропдауна
     `overflow-y: auto`, поэтому вложенная панель обрезалась бы собственным родителем.
     Открытие рекурсивно, глубина бесплатна — этим и решается overflow-панель «…»:
     каждая невлезшая секция становится строкой-флайаутом (`Project ▸`, `Window ▸`), а её
     собственные подменю просто уезжают на уровень глубже (`Node ▸ Align ▸`). Плоский
     вариант отвергнут: он вернул бы «одну простыню», от которой уходит весь проход.
   - **У строки-флайаута свои метаданные: лейбл и слот в одной таблице.** Строка подменю —
     не команда (нет id, нет обработчика, нет своего `menuOrder`), и держать эти два поля
     ей больше негде, поэтому `SUBMENU_LABELS` заменён на `SUBMENU_ROWS: Record<fullPath,
     { label, menuOrder }>` в `CommandRegistry` (`node/align` → 200, `node/distribute` →
     210). Это не второй источник правды: слот хранится **один раз** и там же, где лейбл.
     `MenuSectionGroup` несёт `menuOrder`, а дропдаун **сливает плоские строки и строки
     подменю по слоту** — авторазделители считаются по объединённой последовательности,
     так что `Node` выходит ровно как в §2.3: 100/110 · 200 `Align ▸` / 210
     `Distribute ▸` · 300/310/320. Незнакомый путь всё равно рисуется: лейбл — капитализация
     последнего сегмента, слот — минимальный `menuOrder` собственных пунктов. Строки типов
     нод в `Create` синтезирует сам компонент, поэтому их слоты (100/110/120/130) объявлены
     там же — `Browse All Nodes…` (900) остаётся под ними за разделителем.
   - Найдено живьём и исправлено там же: компонент строил `menuSections` один раз в
     `connectedCallback` и **навсегда** отдавал этот снимок — `Node ▸ Align` в DOM не
     доходил вообще. `CommandRegistry` получил `onDidChangeCommands()` (одно уведомление
     на `registerMany`), компонент — пересборку по нему (склейка через микротаск) и по
     жесту открытия. Спека сверяет DOM с `buildMenuSections()`, а не «есть хоть какая-то
     разметка».
...
- **Свёрнутость по умолчанию** — только когда схема просит: `groups[name].expanded === false`
  (поле уже есть, надо начать его читать). `Node`/`Transform`/`Layout` не сворачиваются никогда,
  что бы схема ни говорила — поэтому `Debug` и `Editor` в кандидатах бессмысленны (они
  вливаются в закреплённый `Node`); кандидаты — второстепенные группы вроде `Limits`,
  `Confiner`, `Trails`, `Sub Emitter`, `Shadow`, `Filtering`, `Damping`.

  Уточнено при реализации G10: поле уже стоит в 12 файлах рантайма, и как только его начали
  читать, дефолты поменялись живьём. Ревизия показала, что все они разумны, кроме одного:
  `UIControl2D` сворачивает `Label` и `Skin`, а у `Label2D` текст — это и есть вся нода.
  Поэтому `Label2D` переопределяет `Label: { expanded: true }`, а `Skin` (необязательная
  подложка) остаётся свёрнутым.

- **Порядок внутри бэнда Layout задан явно** (`Size` → `Anchors` → `Flow`), а не порядком
  объявления: это порядок чтения «какого я размера → где я стою → как раскладываю детей», а
  `Node2D` объявляет флоу первым, что читается наоборот. В остальных бэндах порядок
  объявления и есть ответ.
...
- **Ребёнок под флоу-родителем.** Проверено по `Node2D.applyFlowLayout()` и
  ...
  **только на ведомую ось** (идиома Unity «driven by LayoutGroup»), а не на оба поля и не
  ...
  Уточнено при реализации G11:
  - Семантика осей перепроверена по `Node2D.applyFlowLayout()` / `resolveHorizontalLayout` /
    `resolveVerticalLayout` живьём в коде — таблица выше подтверждена без правок.
  - Имена групп и лейблы **нигде не персистятся**: `.pix3scene` пишет только имена свойств,
    а единственные читатели `prop.ui.group` — сам инспектор и одна спека `Label2D`. Поэтому
    `'Anchor'`→`'Anchors'` и `'Cross Align'`→`'Cross Axis'` — правка без миграции сцен.
    `SECTION_ALIAS`/`LAYOUT_MEMBER_RANK` продолжают принимать оба написания, чтобы редактор
    поверх более старого рантайма не терял подблок.
  - `.inspector-callout` в G8 не завёлся — примитив добавлен здесь, в тот же
    `inspector-controls.ts.css` и так же под `pix3-inspector-panel`. Рядом появился
    `.inspector-subsection__note`: у `__hint` роль «тела нет, включи меня», а тут нужна
    неинтерактивная строка-оговорка при **показанном** теле (пропавший `stretch`).
  - Per-axis disable потребовал `disabledAxes` + `disabledAxisTitle` у `pix3-vector2-editor`:
    у него был только общий `disabled`, а гасить надо ровно одно поле из двух.
  - `renderSizeGroup` распался на «контент» и «секцию»: внутри Layout ряд Size рисуется без
    собственного заголовка, а на не-2D ноде `Size` по-прежнему сама себе секция.
